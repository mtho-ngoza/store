...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    /**
     * Find all orders with customer eagerly fetched.
     * Prevents N+1 query: instead of 1 + N queries, executes only 1 query with JOIN.
     *
     * @return list of all orders with customers
     */
    @EntityGraph(attributePaths = {"customer"})
    List<Order> findAll();

    /**
     * Find order by ID with customer eagerly fetched.
     * Prevents N+1 query when accessing order.getCustomer().
